<template>
  <AdminEntityForm
    mode="create"
    title="Tạo banner"
    subtitle="Thêm banner mới để hiển thị trên website"
...
const schema = [
  ...
  },
  {
    ...
    help: "Bạn có thể upload ảnh hoặc nhập trực tiếp path/url.",
  },
];
...
function validate(formValues) {
  ...
  if (
    ...
    Number.isNaN(new Date(formValues.starts_at).getTime())
  ) {
  ...
  if (
    formValues.ends_at &&
    Number.isNaN(new Date(formValues.ends_at).getTime())
  ) {
...
async function handleSubmit(payload) {
  try {
    ...
    const msg =
      e?.response?.data?.message || e?.message || "Tạo banner thất bại.";
