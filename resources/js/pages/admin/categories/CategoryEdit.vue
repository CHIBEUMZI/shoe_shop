...
const filteredParents = computed(() => {
  ...
  const filtered = list.filter((c) => String(c.id) !== String(id.value));
...
const selectedParent = computed(() => {
  ...
  return parentOptions.value.find((x) => String(x.id) === String(values.value.parent_id)) || null;
});

async function fetchParents(showErrorToast = true) {
...
async function loadCategory(categoryId) {
  try {
    ...
    const { data } = await categoryAdminService.show(categoryId);
    ...
    return {
      ...
      status: Number(c.status ?? 1) === 1,
    };
  } catch (e) {
    const msg =
      e?.response?.data?.message || e?.message || "Không tải được thông tin danh mục.";
...
async function onSubmit(payload) {
  try {
    await categoryAdminService.update(id.value, payload);
    ...
    const msg =
      e?.response?.data?.message || e?.message || "Cập nhật danh mục thất bại.";
  ...
  >
    <template #field:slug="{ values: v, errors }">
      <div class="field full">
        ...
        />
        <div class="hint">Bạn đang ở chế độ sửa, slug mặc định không tự đổi theo tên.</div>
