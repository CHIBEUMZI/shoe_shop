...
/**
 * matrixChecked: Set<"Color__Size"> — tập hợp các ô đang được tick
 * variantCache:  Map<"Color__Size", variantData> — giữ price/stock khi untick rồi tick lại
 */
...
/** Toggle toàn bộ 1 hàng (màu) */
function toggleRow(color) {
  const allOn = ALL_SIZES.every((s) => matrixChecked.value.has(matrixKey(color, s)));
  const next = new Set(matrixChecked.value);
  ALL_SIZES.forEach((s) => {
    if (allOn) next.delete(matrixKey(color, s));
    else next.add(matrixKey(color, s));
  });
  matrixChecked.value = next;
}

/** Toggle toàn bộ 1 cột (size) */
function toggleCol(size) {
  const allOn = ALL_COLORS.value.every((c) => matrixChecked.value.has(matrixKey(c, size)));
  const next = new Set(matrixChecked.value);
  ALL_COLORS.value.forEach((c) => {
    if (allOn) next.delete(matrixKey(c, size));
    else next.add(matrixKey(c, size));
  });
  matrixChecked.value = next;
}

/** Trạng thái indeterminate/checked cho header hàng */
...
/** Trạng thái indeterminate/checked cho header cột */
function colState(size) {
  const count = allColors.value.filter((c) => matrixChecked.value.has(matrixKey(c, size))).length;
  return { checked: count === allColors.value.length, indeterminate: count > 0 && count < allColors.value.length };
}
...
watch(
  () => matrixChecked.value,
  (checked) => {
    // build ordered list: màu trước, size sau
    const desired = [];
    for (const color of allColors.value) {
      for (const size of ALL_SIZES) {
        if (checked.has(matrixKey(color, size))) {
          desired.push({ color, size });
        }
      }
    }
    ...
    const nextVariants = desired.map(({ color, size }) => {
      const key = matrixKey(color, size);
      // tìm variant hiện có (giữ nguyên price/stock/images...)
      const existing = values.value.variants.find(
        (v) => v.color === color && v.size === size
      );
      if (existing) return existing;

      // lấy từ cache nếu đã nhập trước đó
      if (variantCache.value.has(key)) {
        return { ...variantCache.value.get(key), _key: makeKey() };
      }
  ...
  (rows) => {
    rows.forEach((r) => {
      ...
      if (lastAuto && cur !== lastAuto) {
        touchedVariantSku.value = { ...touchedVariantSku.value, [key]: true };
      }
    });
  },
  { deep: true }
...
const schema = computed(() => [
  ...
  { group: "general", name: "slug", label: "Slug (Tiếng Việt)", type: "text", required: true, placeholder: "Nhập slug (để trống sẽ tự tạo)", help: "Slug sẽ được tự động tạo từ tên nếu để trống (xóa trống để auto lại)." },
  { group: "general", name: "sku", label: "SKU (Auto từ tên - có thể sửa)", type: "text", placeholder: "VD: NIKE-AIR-FORCE-1", help: "Nếu để trống, hệ thống sẽ tự tạo theo tên. Nếu bạn sửa tay, hệ thống sẽ không tự đổi nữa (xóa trống để auto lại)." },
  ...
  { group: "media", groupTitle: "Media", name: "thumbnail", label: "Hình ảnh", type: "file", full: true, upload: (file) => uploadAdminService.uploadProductImage(file), uploadHint: "PNG, JPG, WEBP (Max 5MB)", placeholder: "path/url..." },
]);
...
function validate(v) {
  ...
  if (!Array.isArray(v.variants) || v.variants.length === 0) return { ...errs, _form: "Cần chọn ít nhất 1 biến thể từ bảng màu × size" };
  const seen = new Set();
...
function buildPayload(v) {
  return {
    ...
    variants: (v.variants || []).map((it) => ({
      ...
      // ảnh lấy từ colorImages theo màu
...
async function onSubmit(payload) {
  try {
    ...
    notify.success("Tạo sản phẩm thành công.", { title: "Lưu thành công", duration: 2500 });
    router.push("/admin/products");
  } catch (e) {
    notify.error(e?.response?.data?.message || e?.message || "Tạo sản phẩm thất bại.", { title: "Lưu thất bại", duration: 3500 });
    throw e;
  }
}
...
/** ================= COLOR IMAGES ================= */
/**
 * colorImages: { [color]: Array<{ url, sort_order }> }
 * Ảnh được quản lý theo màu, dùng chung cho mọi size của màu đó.
 * Cache lại khi màu bị bỏ chọn để không mất dữ liệu.
 */
const colorImages = ref({});
const colorImagesCache = ref({});

/** Danh sách màu đang được chọn (có ít nhất 1 size) */
const activeColors = computed(() =>
  allColors.value.filter((c) =>
    ALL_SIZES.some((s) => matrixChecked.value.has(matrixKey(c, s)))
  )
);

/** Khi activeColors thay đổi: init entry mới, cache entry bị xóa */
watch(
  activeColors,
  (colors) => {
    ...
    colors.forEach((c) => {
      if (!next[c]) {
        next[c] = colorImagesCache.value[c]
          ? [...colorImagesCache.value[c]]
          : [{ url: "", sort_order: 0 }];
      }
    });
    // cache và xóa màu không còn active
    Object.keys(next).forEach((c) => {
      if (!colors.includes(c)) {
        colorImagesCache.value[c] = next[c];
        delete next[c];
      }
    });
...
function addColorImage(color) {
  ...
  colorImages.value = {
    ...colorImages.value,
    [color]: [...imgs, { url: "", sort_order: nextOrder }],
  };
              ...
              <tbody>
                <tr v-for="color in ALL_COLORS" :key="color">
                  ...
                  <td
                    ...
                    @click="toggleRow(color)"
                    :title="`Toggle tất cả size cho màu ${color}`"
                  >
                  ...
                  <td
                    ...
                    :key="size"
                    class="matrix-cell"
                    :class="{ checked: isChecked(color, size) }"
                    @click="toggleCell(color, size)"
                    :title="`${color} / ${size}`"
                  >
            ...
            <table class="variants">
              ...
              <tbody>
                <template v-for="(it, i) in v.variants" :key="it._key || i">
                  <tr>
                    <!-- màu (readonly — do matrix quản lý) -->
                    <td>
                    ...
                    <!-- size (readonly) -->
                    <td>
                      <span class="size-chip">{{ it.size }}</span>
                    </td>
                    ...
                    <td class="num">
                      <input v-model.number="it.sale_price" type="number" min="0" class="tcontrol" placeholder="—" />
                    </td>
                    <td class="num">
                    ...
                    <td class="center">
                      <input v-model="it.is_active" type="checkbox" />
                    </td>

                    <!-- untick = xóa khỏi matrix -->
                    <td class="center">
                      <button
                        class="icon-btn danger"
                        type="button"
                        title="Bỏ chọn biến thể này"
                        @click="toggleCell(it.color, it.size)"
                      >
                        <span class="material-symbols-outlined">close</span>
                      </button>
                    </td>
              ...
              >
                ...
                <div class="fieldx">
                  ...
                  <label class="filebtn">
                    ...
                    <input
                      ...
                      type="file"
                      accept="image/*"
                      @change="(e) => onPickColorImageFile(e, color, j)"
                    />
                  </label>
                ...
                <div class="img-actions">
                  <button
                    ...
                    type="button"
                    title="Xóa ảnh"
                    @click="removeColorImage(color, j)"
                  >
...
/* ===== IMAGES ===== */
.imgs-cell { background: #f9fbff; padding: 16px !important; }
.imgs-head { display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px; }
.imgs-title {
  font-weight: 700; color: #374151; font-size: 12.5px;
  display: flex; align-items: center; gap: 6px;
}
.imgs-title::before { content: '🖼'; }
.imgs-grid { display: grid; gap: 12px; }

.img-row {
  display: grid;
  grid-template-columns: 90px 220px 1fr 120px 56px;
  gap: 12px;
  align-items: start;
  padding: 12px;
  border: 1.5px solid #e8ecf4;
...
.btn-add-color .material-symbols-outlined {
  font-size: 18px;
}
