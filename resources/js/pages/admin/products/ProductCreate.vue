...
/** detect manual edit variant sku */
watch(
...
const schema = computed(() => [
  ...
  },
  {
    ...
    help: "Slug sẽ được tự động tạo từ tên nếu để trống (xóa trống để auto lại).",
  },
  {
    group: "general",
    name: "sku",
    ...
    help: "Nếu để trống, hệ thống sẽ tự tạo theo tên. Nếu bạn sửa tay, hệ thống sẽ không tự đổi nữa (xóa trống để auto lại).",
  },
  {
    group: "general",
  ...
  },
  {
    ...
    type: "file",
    full: true,
    upload: (file) => uploadAdminService.uploadProductImage(file),
...
async function onSubmit(payload) {
  try {
    ...
    const msg =
      e?.response?.data?.message || e?.message || "Tạo sản phẩm thất bại.";
...
function removeVariant(i) {
  const cur = values.value.variants || [];
  if (cur.length <= 1) {
  ...
  values.value = { ...values.value, variants: nextArr };
...
function addImage(variantIndex) {
  const variants = [...(values.value.variants || [])];
  ...
  values.value = { ...values.value, variants };
                    ...
                    <td colspan="8" class="imgs-cell">
                      <div class="imgs-head">
                        <div class="imgs-title">
                          Images for #{{ i + 1 }} ({{ it.color }} / {{ it.size }})
                        </div>
                      ...
                      <div class="imgs-grid">
                        <div class="img-row" v-for="(img, j) in it.images" :key="j">
                          ...
                          <div class="fieldx">
                            ...
                            <div class="hintx" v-if="uploadingVariantImage[`${i}-${j}`]">
                              Uploading...
                            </div>
                          </div>
...
.card-subtitle{ margin-top:2px; font-size:12px; color:#94a3b8; font-weight:500; }
